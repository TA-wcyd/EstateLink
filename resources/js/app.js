...
const state = {
  ...
  compareSelection: (() => {
    try {
      const stored = JSON.parse(localStorage.getItem('estatelink_compare_selection') || '[]');
...
function handleRoute(path) {
  ...
  switch (path) {
    case '/properties/compare':
      showView('view-property-compare');
...
function getCompareSelectionSummaryText() {
  return `${state.compareSelection.length} of 4 selected`;
}

function persistCompareSelection() {
  localStorage.setItem('estatelink_compare_selection', JSON.stringify(state.compareSelection));
  const summary = document.getElementById('compare-selected-summary');
  if (summary) summary.textContent = getCompareSelectionSummaryText();

  const compareButton = document.getElementById('compare-open-selected-btn');
  if (compareButton) compareButton.disabled = state.compareSelection.length < 2;
}

async function loadCompareSelectorProperties() {
  const search = document.getElementById('compare-selector-search')?.value.trim() || '';
  const propertyType = document.getElementById('compare-selector-type')?.value || '';
  ...
  try {
    ...
    if (!response.ok) throw new Error('Unable to load compare properties');

    const result = await response.json();
    const properties = result.data || [];
    const list = document.getElementById('compare-selector-results');
    if (!list) return;

    if (properties.length === 0) {
      list.innerHTML = `
        <div class="state-box" style="padding: 30px 18px;">
          <div class="state-icon">🏠</div>
          <h3 style="font-size: 1rem; margin-bottom: 8px;">No matching properties</h3>
          <p>Try a different title or location.</p>
        </div>
      `;
      return;
    }
    ...
    list.innerHTML = properties.map(property => {
      const selected = state.compareSelection.includes(property.id);
      const disabled = !selected && state.compareSelection.length >= 4;
      const buttonLabel = selected ? '✓ Selected' : '+ Compare';
      const typeLabel = (property.property_type || 'property').replace(/_/g, ' ');
      const fallbackImage = '/images/hero_building.jpg';
      const imageUrl = property.main_image || fallbackImage;

      return `
        <div class="compare-selector-item">
          <img src="${imageUrl}" alt="${escapeHtml(property.title)}" class="compare-selector-image" onerror="this.src='${fallbackImage}'">
          <div class="compare-selector-details">
            <div class="compare-selector-title-row">
              <div class="compare-selector-name">${escapeHtml(property.title)}</div>
              <button class="btn ${selected ? 'btn-primary' : 'btn-secondary'} btn-sm" type="button" ${disabled ? 'disabled' : ''} onclick="toggleCompareSelection(${property.id})">
                ${buttonLabel}
              </button>
            </div>
            <div class="compare-selector-meta">📍 ${escapeHtml(property.location || 'Not Available')}</div>
            <div class="compare-selector-meta">🏷️ ${escapeHtml(typeLabel)}</div>
            <div class="compare-selector-meta">💰 ${formatCurrency(property.price)} • 🛏️ ${property.bedrooms ?? 'N/A'} • 🚿 ${property.bathrooms ?? 'N/A'} • 📐 ${property.size ?? 'N/A'} sqft</div>
            <div class="compare-selector-meta compare-status">${property.verification_status ? escapeHtml(property.verification_status) : 'Not Available'}</div>
          </div>
        </div>
      `;
    }).join('');
    ...
    console.error('Compare selector load error:', error);
    const list = document.getElementById('compare-selector-results');
    if (list) {
      list.innerHTML = `
        <div class="state-box" style="padding: 30px 18px;">
          <div class="state-icon">⚠️</div>
          <h3 style="font-size: 1rem; margin-bottom: 8px;">Unable to load listings</h3>
...
window.openCompareSelectorModal = function () {
  closeAllModals();
  const modal = document.getElementById('modal-property-compare-selector');
  if (!modal) return;

  modal.classList.add('active');
  persistCompareSelection();
  loadCompareSelectorProperties();
};
...
window.toggleCompareSelection = function (id) {
  ...
  if (state.compareSelection.includes(propertyId)) {
    state.compareSelection = state.compareSelection.filter(value => value !== propertyId);
  } else {
    if (state.compareSelection.length >= 4) {
      showToast('You can compare up to 4 properties at a time.', 'info');
      return;
    }
    state.compareSelection.push(propertyId);
  }
...
window.removeCompareSelection = function (id) {
  state.compareSelection = state.compareSelection.filter(value => value !== Number(id));
  persistCompareSelection();
  renderComparisonPage();
...
window.renderComparisonPage = async function () {
  const container = document.getElementById('comparison-page-content');
  if (!container) return;

  if (state.compareSelection.length === 0) {
    container.innerHTML = `
      <div class="comparison-empty state-box">
        <div class="comparison-empty-icon">▣</div>
        <h2>Your Comparison Matrix is Empty</h2>
        <p>Select 2 or more verified properties to compare price, area, location, and verification details side-by-side.</p>
        <div class="comparison-empty-actions">
          <button class="btn btn-primary" type="button" onclick="openCompareSelectorModal()">Select Properties to Compare</button>
          <button class="btn btn-secondary" type="button" onclick="navigateTo('/properties')">Browse All Properties</button>
        </div>
      </div>
    `;
    return;
  ...
  if (state.compareSelection.length < 2) {
    const propertyIds = [...state.compareSelection];
    const cards = propertyIds.map(id => {
      const property = state.compareSelection.length ? null : null;
      return `
        <div class="compare-selection-card">
          <div class="compare-selection-card-head">
            <div class="compare-selection-card-title">Property #${id}</div>
            <button class="btn btn-secondary btn-sm" type="button" onclick="removeCompareSelection(${id})">Remove</button>
          </div>
        </div>
      `;
    }).join('');
    ...
    container.innerHTML = `
      <div class="comparison-shell">
        <div class="comparison-message">
          Select at least one more property to begin comparing. You can compare 2 to 4 properties at once.
        </div>
        <div class="comparison-selection-strip">${cards}</div>
        <div class="comparison-empty-actions" style="margin-top: 20px;">
          <button class="btn btn-primary" type="button" onclick="openCompareSelectorModal()">Add More Properties</button>
          <button class="btn btn-secondary" type="button" onclick="clearCompareSelection()">Clear Selection</button>
        </div>
      </div>
    `;
    return;
  ...
  try {
    const response = await fetch(`/api/properties/compare?ids=${state.compareSelection.join(',')}`, {
      headers: { 'Accept': 'application/json' }
    });

    const data = await response.json();
    if (!response.ok) {
      throw new Error(data.message || 'Unable to load comparison data');
    }

    const properties = data.properties || [];
    if (properties.length < 2) {
      throw new Error('Select at least two approved properties to compare.');
    }

    const propertyCardsHtml = properties.map(property => `
      <div class="compare-selection-card">
        <div class="compare-selection-card-head">
          <span class="compare-selection-pill">${escapeHtml((property.property_type || 'property').replace(/_/g, ' '))}</span>
          <button class="compare-remove-btn" type="button" onclick="removeCompareSelection(${property.id})">Remove</button>
        </div>
        <img src="${property.main_image || '/images/hero_building.jpg'}" alt="${escapeHtml(property.title)}" class="compare-card-image" onerror="this.src='/images/hero_building.jpg'">
        <div class="compare-card-title">${escapeHtml(property.title)}</div>
        <div class="compare-card-meta">📍 ${escapeHtml(property.location || 'Not Available')}</div>
        <div class="compare-card-meta">💰 ${formatCurrency(property.price)}</div>
      </div>
    `).join('');

    const getCellValue = (property, fallback) => {
      if (property === undefined || property === null || property === '') return fallback;
      return property;
    };

    const rows = [
      ['Property name', properties.map(property => `<strong>${escapeHtml(property.title || 'Not Available')}</strong>`)],
      ['Property type', properties.map(property => escapeHtml((property.property_type || 'Not Available').replace(/_/g, ' ')))],
      ['Location', properties.map(property => escapeHtml(property.location || 'Not Available'))],
      ['Address', properties.map(property => escapeHtml(property.address || 'Not Available'))],
      ['Verification status', properties.map(property => escapeHtml((property.verification_status || 'Not Available').toUpperCase()))],
      ['Price', properties.map(property => formatCurrency(property.price))],
      ['Price per sq ft', properties.map(property => property.price_per_sqft !== null && property.price_per_sqft !== undefined ? `${formatCurrency(property.price_per_sqft)}` : 'Not Available')],
      ['Area / Sq Ft', properties.map(property => property.size !== null && property.size !== undefined && property.size !== '' ? `${Number(property.size).toLocaleString('en-IN')} sq ft` : 'Not Available')],
      ['Bedrooms', properties.map(property => property.bedrooms !== null && property.bedrooms !== undefined ? `${property.bedrooms}` : 'Not Available')],
      ['Bathrooms', properties.map(property => property.bathrooms !== null && property.bathrooms !== undefined ? `${property.bathrooms}` : 'Not Available')],
      ['Transaction status', properties.map(property => escapeHtml((property.transaction_status || 'Not Available').replace(/_/g, ' ')))],
      ['Seller / contact', properties.map(property => escapeHtml(property.phone || 'Not Available'))]
    ];

    const tableRowsHtml = rows.map(([label, values]) => `
      <tr>
    ...
    container.innerHTML = `
      <div class="comparison-shell">
        <div class="comparison-toolbar">
          <div>
            <div class="comparison-toolbar-title">Selected Properties</div>
            <div class="comparison-toolbar-subtitle">${properties.length} properties ready for comparison</div>
          </div>
          <div class="comparison-toolbar-actions">
            <button class="btn btn-primary btn-sm" type="button" onclick="openCompareSelectorModal()">Add More</button>
            <button class="btn btn-secondary btn-sm" type="button" onclick="clearCompareSelection()">Clear All</button>
          </div>
        </div>

        <div class="comparison-selection-strip">${propertyCardsHtml}</div>
        ...
        <div class="compare-table-wrap">
          <table class="compare-table">
            <thead>
              <tr>
                <th>Attribute</th>
                ${properties.map(property => `<th>${escapeHtml(property.title || 'Property')}</th>`).join('')}
              </tr>
            </thead>
            <tbody>
              ${tableRowsHtml}
    ...
    container.innerHTML = `
      <div class="comparison-shell">
        <div class="comparison-message">${escapeHtml(error.message || 'Unable to load comparison data.')}</div>
        <div class="comparison-empty-actions">
          <button class="btn btn-primary" type="button" onclick="openCompareSelectorModal()">Select Properties to Compare</button>
          <button class="btn btn-secondary" type="button" onclick="clearCompareSelection()">Clear Selection</button>
        </div>
